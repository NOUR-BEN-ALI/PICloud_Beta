...
import arctic.example.pi.dto.AssociationDonationCountDto;
...
public interface DonationsRepostitory extends JpaRepository<Donations,Long> {
  ...
  List<Donations> findDonationsByAssociations_Id(Long id);
    @Query("SELECT MONTH(d.date), COUNT(d) FROM Donations d WHERE YEAR(d.date) = YEAR(CURDATE()) GROUP BY MONTH(d.date)")
    List<AssociationDonationCountDto> countByMonth();
